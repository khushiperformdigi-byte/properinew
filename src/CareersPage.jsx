...
function JobImage({ url, title = '', department = '' }) {
  ...
  if (url && !imgError) {
    return (
      <div className="w-12 h-12 sm:w-28 sm:h-28 lg:w-32 lg:h-32 rounded-xl sm:rounded-2xl overflow-hidden bg-purple-50 border border-purple-100 shrink-0 shadow-xs sm:shadow-md group-hover:scale-[1.02] transition-transform flex items-center justify-center">
        <img
          src={url}
          alt={title}
  ...
  return (
    <div className={`w-12 h-12 sm:w-28 sm:h-28 lg:w-32 lg:h-32 rounded-xl sm:rounded-2xl ${badge.bg} text-white shrink-0 shadow-xs sm:shadow-md flex flex-col items-center justify-center p-1 sm:p-3 group-hover:scale-[1.02] transition-transform text-center`}>
      <span className="text-xl sm:text-4xl lg:text-5xl drop-shadow-xs">{badge.icon}</span>
      <span className="hidden sm:inline-block text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider mt-1.5 opacity-95 leading-tight">{badge.label}</span>
    </div>
  );
}
...
export default function CareersPage({ onNavigateHome, onNavigatePage }) {
  ...
  useEffect(() => {
    if (mobileMenuOpen || selectedJob || showThankYouModal || talkAdvisorModal || mobileFilterOpen) {
  ...
  }, [mobileMenuOpen, selectedJob, showThankYouModal, talkAdvisorModal, mobileFilterOpen]);
  ...
  return (
    <div className="w-full bg-[#FAF8FC] font-sans text-[#1E1B2E] antialiased selection:bg-purple-100 selection:text-[#7C1FA8]">
      ...
      {/* 3. HERO BANNER SECTION (COMPACT LAYOUT WITH RICH SUBTEXT) */}
      <section className="w-full bg-gradient-to-b from-[#FAF6FC] via-white to-[#FAF8FC] border-b border-[#EBE8EF] py-6 sm:py-8 lg:py-10 px-4 sm:px-6 lg:px-8 overflow-hidden select-none">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-6 lg:gap-12">
          
          {/* Left Column: Title & Subtitle */}
          <div className="w-full lg:max-w-[55%] flex flex-col text-left">
            <h1 className="font-sans font-extrabold text-[32px] sm:text-[38px] lg:text-[44px] leading-[1.15] text-black tracking-[-1px] mb-3">
              Build Your Career<br />With Prosperi5
            </h1>
            <p
              style={{ fontFamily: "'Inter', sans-serif" }}
              className="font-medium text-[14px] sm:text-[15.5px] lg:text-[16.5px] leading-[24px] sm:leading-[26px] text-gray-800 tracking-[-0.3px] max-w-xl"
            >
              We are always looking for passionate, driven and talented individuals to join our mission of helping people achieve financial freedom. Explore our open positions, discover our collaborative culture, and take the next step in building a rewarding career with continuous growth and impact.
            </p>
          </div>

          {/* Right Column: Compact Vector Illustration matching reference screenshot */}
          <div className="w-full lg:w-[45%] flex justify-center lg:justify-end items-center relative py-2">
            <div className="relative w-[320px] sm:w-[400px] h-[220px] sm:h-[250px] flex items-center justify-center">
              
              {/* Dotted Circle Orbit behind Briefcase */}
              <div className="absolute left-[3%] sm:left-[6%] top-[12%] w-[140px] sm:w-[170px] h-[140px] sm:h-[170px] rounded-full border-2 border-dashed border-[#C895EC]/60 opacity-80"></div>

              {/* Briefcase Icon Card (Left) */}
              <div className="absolute left-[8%] sm:left-[10%] top-[22%] z-20 bg-white p-3 sm:p-3.5 rounded-2xl shadow-lg border border-purple-100/90 hover:scale-105 transition-transform duration-300">
                <svg className="w-10 h-10 sm:w-14 sm:h-14 text-[#1E1135]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M20 7h-4V4a1 1 0 00-1-1H9a1 1 0 00-1 1v3H4a1 1 0 00-1 1v11a1 1 0 001 1h16a1 1 0 001-1V8a1 1 0 00-1-1zM10 4h4v3h-4V4zm-1 8.5h6m-6 3h6" />
                </svg>
              </div>

              {/* Center Soft Purple Circle Backdrop with Executive Chair Icon */}
              <div className="relative z-10 w-[160px] sm:w-[195px] h-[160px] sm:h-[195px] rounded-full bg-[#F3EBF9] border border-purple-200/60 flex items-center justify-center shadow-inner">
                {/* Executive Chair SVG Icon */}
                <svg className="w-20 h-20 sm:w-28 sm:h-28 text-[#1E1135] filter drop-shadow-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  {/* Headrest & Backrest */}
                  <rect x="7" y="2" width="10" height="9" rx="2" strokeWidth="1.6" />
                  {/* Cushion Seat */}
                  <rect x="5" y="12" width="14" height="3" rx="1.5" strokeWidth="1.6" />
                  {/* Armrests */}
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M4 12V9.5a1.5 1.5 0 011.5-1.5M20 12V9.5a1.5 1.5 0 00-1.5-1.5" />
                  {/* Support Stem */}
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M12 15v3.5M9 20.5l3-2 3 2M12 18.5v2" />
                  {/* Wheels */}
                  <circle cx="8" cy="20.5" r="1" strokeWidth="1.6" />
                  <circle cx="16" cy="20.5" r="1" strokeWidth="1.6" />
                  <circle cx="12" cy="21" r="1" strokeWidth="1.6" />
                </svg>
              </div>

              {/* Magnifying Glass Icon Card (Top Right) */}
              <div className="absolute right-[6%] sm:right-[8%] top-[6%] z-20 bg-white p-2.5 sm:p-3 rounded-xl shadow-md border border-purple-100/90 hover:scale-110 transition-transform duration-300">
                <svg className="w-5 h-5 sm:w-7 sm:h-7 text-[#1E1135]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>

              {/* Org Hierarchy Icon Card (Bottom Right) */}
              <div className="absolute right-[4%] sm:right-[6%] bottom-[8%] z-20 bg-white p-2.5 sm:p-3 rounded-xl shadow-md border border-purple-100/90 hover:scale-105 transition-transform duration-300">
                <svg className="w-6 h-6 sm:w-8 sm:h-8 text-[#1E1135]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.6" d="M12 4.5v15m0-15l-4 4m4-4l4 4M7 19.5h10M9 16.5h6" />
                  <circle cx="12" cy="7" r="1.5" strokeWidth="1.6" />
                  <circle cx="8" cy="17" r="1.5" strokeWidth="1.6" />
                  <circle cx="16" cy="17" r="1.5" strokeWidth="1.6" />
                </svg>
              </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* LEFT COLUMN: UPGRADED STICKY SIDEBAR FILTER (DESKTOP ONLY) */}
          <aside className="hidden lg:block lg:col-span-4 bg-white rounded-[24px] border border-[#EBE8EF] shadow-md sticky top-24 z-20 self-start overflow-hidden">
            {/* Header Banner */}
          ...
          <section className="lg:col-span-8 space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
              <div>
                ...
                <p className="text-xs text-[#8E8A9D] mt-0.5 sm:mt-1">
                  Click on any position card to view full job responsibilities and qualifications.
                </p>
              </div>

              {/* MOBILE HAMBURGER / FILTER BUTTON */}
              <button
                onClick={() => setMobileFilterOpen(true)}
                className="lg:hidden flex items-center gap-2 bg-[#7C1FA8] hover:bg-[#6b1a91] text-white text-xs font-extrabold px-4 py-2.5 rounded-xl shadow-md transition-all cursor-pointer active:scale-95 shrink-0"
              >
                <svg className="w-4 h-4 stroke-[2.5]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h7" />
                </svg>
                <span>Filter Positions ({(searchQuery || selectedDept !== 'All' || selectedLoc !== 'All' || selectedExp !== 'All') ? 'Active' : filteredJobs.length})</span>
              </button>
            </div>
              ...
              {!jobsLoading && paginatedJobs.map((job) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-2xl sm:rounded-[24px] border p-4 sm:p-7 shadow-sm transition-all duration-300 cursor-pointer ${
                      isExpanded 
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row items-start gap-3.5 sm:gap-6">
                      
                      {/* Top Row on Mobile: Image + Title + Apply Button */}
                      <div className="flex items-center sm:items-start gap-3 w-full sm:w-auto">
                        ...
                        {/* Mobile Only: Title & Apply Button next to image */}
                        <div className="flex-1 min-w-0 sm:hidden">
                          <div className="flex items-center justify-between gap-1.5 mb-1">
                            <h2 className="text-xs font-extrabold text-[#1E1B2E] leading-tight line-clamp-2">
                              {job.title}
                            </h2>
                          ...
                          <button
                            ...
                            className="bg-[#7C1FA8] hover:bg-[#6b1a91] text-white font-bold text-[10.5px] px-3 py-1 rounded-lg shadow-2xs active:scale-95 transition-all mt-0.5 cursor-pointer"
                          >
                            Apply Now
                          </button>
                        </div>
                      ...
                      <div className="flex-1 min-w-0 w-full">
                        ...
                        <div className="hidden sm:flex flex-row items-center justify-between gap-3 mb-2">
                          ...
                          <button
                            onClick={(e) => handleApplyClick(job, e)}
                            className="px-5 py-2 rounded-xl border border-[#7C1FA8] text-[#7C1FA8] hover:bg-[#7C1FA8] hover:text-white font-bold text-xs sm:text-sm transition-all duration-200 cursor-pointer shadow-2xs whitespace-nowrap"
                          >
                            Apply Now
                          </button>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 sm:gap-4 text-[10.5px] sm:text-xs text-[#544F66] mb-2 sm:mb-3">
                          {/* Location */}
                          <div className="flex items-center gap-1 bg-[#FAF8FC] sm:bg-transparent px-2 py-0.5 sm:p-0 rounded-md sm:rounded-none border sm:border-0 border-[#EBE8EF]">
                            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#7C1FA8] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* MOBILE LEFT SIDEBAR FILTER DRAWER */}
      {mobileFilterOpen && (
        <div className="fixed inset-0 z-[200] lg:hidden font-sans">
          ...
          {/* Left Slide-Over Sidebar Container */}
          <div className="fixed inset-y-0 left-0 w-[310px] max-w-[85vw] bg-white z-[201] shadow-2xl flex flex-col justify-between overflow-y-auto border-r border-[#EBE8EF]">
            <div>
              {/* Header Banner */}
              <div className="bg-gradient-to-r from-[#7C1FA8] to-[#5E1083] p-4 text-white flex items-center justify-between sticky top-0 z-10 shadow-sm">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-lg bg-white/15 flex items-center justify-center backdrop-blur-xs">
                    <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
                ...
                <button
                  onClick={() => setMobileFilterOpen(false)}
                  className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 text-white font-bold flex items-center justify-center text-sm cursor-pointer transition-colors"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="p-4 space-y-4">
                ...
                {/* Counter */}
                <div className="pt-3 border-t border-[#EBE8EF] flex items-center justify-between text-xs text-[#8E8A9D]">
                  <span className="font-medium">Matching Positions:</span>
                  <span className="font-extrabold text-[#7C1FA8] text-xs bg-[#FAF5FD] px-3 py-1 rounded-full border border-purple-200 shadow-2xs">
                    {filteredJobs.length} {filteredJobs.length === 1 ? 'Position' : 'Positions'}
                  </span>
                </div>
            ...
            <div className="p-4 border-t border-[#EBE8EF] bg-[#FAF8FC] space-y-2 sticky bottom-0 z-10">
              <button
                onClick={() => setMobileFilterOpen(false)}
                className="w-full bg-[#7C1FA8] hover:bg-[#6b1a91] text-white font-bold py-3 rounded-xl text-xs uppercase tracking-wider transition-all shadow-md cursor-pointer text-center active:scale-95"
              >
              ...
              {(searchQuery || selectedDept !== 'All' || selectedLoc !== 'All' || selectedExp !== 'All') && (
                <button
                  ...
                  className="w-full bg-white border border-[#EBE8EF] hover:bg-gray-50 text-[#7C1FA8] font-bold py-2.5 rounded-xl text-xs transition-all cursor-pointer text-center"
                >
                  Reset All Filters
                </button>
              )}
