...
export default function BlogPage({ onNavigateHome, onNavigatePage }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          // Fallback to high-quality default static articles on CORS/Network failure
          setBlogPosts(DEFAULT_9_BLOGS);
          setPostsError('');
        }
      } finally {
  ...
  return (
    <div className="w-full bg-[#FAF8FC] font-sans text-[#1E1B2E] antialiased selection:bg-purple-100 selection:text-[#7C1FA8] overflow-x-hidden">
      ...
      {/* HERO BANNER SECTION */}
      <section className="w-full overflow-hidden bg-[#FAF8FC] bg-gradient-to-r from-[#FAF8FC] via-[#F5EEFC] to-[#FAF8FC] border-b border-[#EBE8EF]/60 -mt-[88px] lg:-mt-[104px] pt-0 pb-6 lg:pb-8">
        <img
